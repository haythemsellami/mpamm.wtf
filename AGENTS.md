...
- **`state.notes` is developer-facing telemetry** (`/api/markets` plus stdout, never the dashboard UI). Each note is `{ ts, level, code, venue?, msg }` (`@shared`: `StateNote`). Raise one with the code that matches the event (`ctx.note(code, msg)` in an adapter, `note`/`noteOnce` in the core); the buffer stamps the time, takes the level from the code (`NOTE_LEVEL`), strips URLs so an RPC key can never leak and drops a verbatim repeat.
  Notes are deliberately **NOT on the WS stream** (see the egress rule below) — read them from REST or the log:

  ```bash
  curl -s localhost:8787/api/markets | jq '.state.notes[] | select(.venue=="poe")'      # one venue
  curl -s localhost:8787/api/markets | jq '.state.notes[] | select(.level=="warn")'     # what wants a human
  curl -s localhost:8787/api/markets | jq '.state.notes[] | select(.code|startswith("rpc."))'
  ```
- **The WS stream is the service's biggest cost centre — treat frame size as a budget.** It fans out to every open tab several times a second, so a field added to a `state` or `quotes` frame is multiplied by (clients × ~6/s × forever). An audit found the state frame carrying the immutable venue registry and the whole `notes` buffer: ~27% of ALL egress, for data the dashboard never rendered (Render bill, 2026-09). The rules that came out of it: the registry ships once on the **hello** frame (`helloFrame`/`tickFrame` in `server.ts`, `StreamState` in `@shared`), maintainer telemetry never ships at all, and anything genuinely per-tick earns its place. Snapshot-shaped payloads belong on REST, which the edge compresses and the client already re-fetches on every reconnect. `server/src/__tests__/stream-egress.test.ts` locks this in.
- **Two RPC clients, and the choice is not cosmetic** (`server/src/chain/rpc.ts`): `publicClient` is the HOT pool (quote loop, fills tail — chosen for distance to the tip) and `archiveClient` is the DEEP pool (volume backfill, markout onboarding, gas, `blockAtOrAfter` — chosen for retention). Anything that reads a block more than ~a day old MUST use `archiveClient` (the archive contract is logs/headers/receipts at any depth **plus** historical `eth_getCode`, which `GasTracker.creationBlock()` bisects from block 0); the tip-freshest nodes are pruning fullnodes and a deep read on the hot pool returns a JSON-RPC error, which by design never trips failover — the cursor just holds forever. Every deep read must also use the availability gate (`guardRpcRead` via `archiveRead`, `GasTracker.rpc`, or `blockAtOrAfter`): the breaker can transparently retry on a backup inside one await, so checking only the returned error can accept a partial/mixed-provider result and consume a cursor. Unset `RPC_ARCHIVE_URL` makes the pools share one client, but deep work still pauses during failover.
