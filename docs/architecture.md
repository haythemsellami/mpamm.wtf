...
Quotes are **block-triggered** by a proposal-aware hot-head watcher (`server/src/chain/heads.ts`). The active HTTP provider selects its configured WebSocket peer, preferring `monadNewHeads` with `newHeads` fallback; a fast `eth_blockNumber` poll (`HEAD_POLL_MS`, default 75ms) always remains active as the no-WebSocket implementation and half-open-connection watchdog. Both paths dedupe into one stream. While a quote pass is in flight, later heads coalesce to the newest observed block so a slow venue cannot build an unbounded queue of stale matrices; any overload is visible as a block-number gap, never as a frame assembled from mixed state.

Every adapter receives that explicit block in `quote(ctx, sizesUsd, blockNumber)`. The scoped transport pins state reads, including **Multicall3 `eth_call`s**, to one resolved block hash with EIP-1898 (POE `getQuote`, Metric `quoteSwap`, Clober `getExpectedOutput`, Hanji's book-assembly helper, Uniswap `quoteExactInputSingle`). The requested matrix is published under the same block after adapters finish or reach their bounded deadline (250ms by default); overdue rows are absent, never copied from an older block. Rows are normalized to `QuoteRow[]` in **bps vs the pair's reference mid**, then annotated with the CEX realized-vs-taker columns — the CEX book is walked for the **same base size** with the taker fee overlaid: realized-vs-realized at size, the only honest comparison for size-sensitive flow. `filledFull=false` marks an exhausted pool/book (`PARTIAL` tag); `oneSided=true` marks a book with only one executable side. The fills tail remains an independent self-scheduling loop (`TAIL_INTERVAL_MS`, default 500ms), so finalized log ingest cannot stretch quote freshness.

The CEX mids/books used for sizing, bps anchors and benchmark rows are captured synchronously once before the frame's first RPC await, so a moving feed cannot give different adapters different reference moments. Quote computation follows the union of subscribed market/size selections, with optional baseline work. HTTP quote calls are unbatched by default (Multicall3 still combines contract reads), or batched per adapter when configured. Head polling has its own unbatched lane. All lanes share the hot pool breaker; tail and archival retries remain independent of quote deadlines. Every published snapshot carries head-observed / quote-started / completed / emitted timestamps, head source, per-adapter wall times, missing venues and coalesced blocks. `state.realtime` rolls those frames into head/quote lag, 60-second block coverage, coalescing and event-loop delay; `/api/health` exposes the same view. The Execution chart positions samples by their real emitted timestamps and breaks paths on missing rows/sides or skipped block numbers — gaps remain visible instead of being compressed into a nominal 300ms sample index, and the wall-time window keeps advancing when the stream stalls.

### Depth curves (BID_ASK_DEPTH)

The same quote path, over a **log-spaced notional grid** ($100 → $1M, `DEPTH_SAMPLES` = 25) instead of the four SIZE pills: `adapter.quote(ctx, depthGrid, blockNumber, requestedMarkets)` → `server/src/depth.ts` slices the result into one executable-spread curve per venue for the requested market. Filtering happens before adapters build their contract calls; opening MON/USDC never prices the rest of the registry. Because the grid lands exactly on every `SIZES_USD` pill, a curve point at $10k uses the same quote math as the QUOTE grid and ROLLING_STATS, not a lookalike. Venue skew survives (the two legs are that venue's two independent walks, never mirrored) and fees stay where the venue puts them (the CEX leg carries its taker fee because `reference.quote()` applies it). A leg ends where the venue stops quoting size — no row, or `filledFull=false` — and nothing is extrapolated past that.

The cost is `DEPTH_SAMPLES / sizesUsd.length` ≈ 6× a four-pill market quote, so depth has a separate **process, scheduler and RPC pool**. One bounded pass groups all demanded markets at the same block hash; per-adapter HTTP batching and 8KB Multicall chunks reduce request overhead. The default interval cap is zero, so each observed head can start work. The first `GET /api/depth/stream?market=` viewer starts that market; later viewers share it, and the last disconnect stops demand. New heads coalesce to the latest block while a pass is running, so stale work cannot queue. Only an already-serialized completed snapshot crosses to the main service, where SSE fanout is a cheap string write; `GET /api/depth?market=` is cache-only and never starts RPC work. The browser labels and fades stale curves after 1.5 seconds; a completed pass explicitly lists missing venues instead of retaining their older prices. Depth state lives inside the curve component so an update does not re-render the rest of the dashboard. Set `RPC_DEPTH_URL` to a separately provisioned tip-fresh node to isolate provider quotas as well as the Node event loop; leaving it unset reuses the hot node and logs that reduced isolation. `DEPTH=off` disables the worker.
...
GET /api/bootstrap?volume=1        lean page bootstrap (no quotes/fills/notes; optional volume)
GET /api/markets                   legacy full snapshot (incl. venues + notes)
GET /api/quotes                    latest complete shared quote matrix (503 if unavailable)
GET /api/quotes/history?market=&size=   last ~60s of real ticks (chart seed)
GET /api/quotes/stats?market=&size=     shared rolling five-minute spread summary
GET /api/depth?market=             last completed depth curve (cache-only snapshot)
GET /api/depth/stream?market=      demand-driven live depth curves (SSE, $100 → $1M)
...
The dashboard uses versioned subscriptions and application-level gzip. A socket negotiates `mpamm.v2.gzip` (or `mpamm.v2.json` when decompression is unavailable), then sends its desired topics. Quotes are filtered before serialization and compressed once per shared topic, independent of viewer count. This works even when a proxy does not negotiate WebSocket compression. Legacy clients still receive full matrices and may negotiate permessage-deflate.

A SharedWorker owns one connection per browser origin and shares the union of visible tabs' topics. A per-tab connection is the fallback. Hidden tabs remove their demand after 60 seconds; returning tabs resynchronize. Execution requests the selected quote market/size and a depth curve; Volume requests volume, Markouts requests fill events, and Leaderboard requests only state plus historical REST results. State is limited to one update per second; the Execution header and frame diagnostics follow the quote topic directly. Proposal replacements invalidate affected server history and reset browser quote samples. Venue metadata is sent on subscription, discovered coverage only on change, and maintainer notes stay on REST.

Each topic has a process epoch and monotonic sequence. Snapshot queues retain at most one pending replacement; fill events remain ordered, and slow sockets are disconnected so REST can repair missed data. Reconnects refresh the page snapshot and fill/history windows. A shared collector quotes every registered market and configured size, including baselines, independently of viewer count. Full quote frames expire after 60 seconds; compact numeric spread samples expire after six minutes. Expiry runs even during upstream outages with no viewers. These buffers are in memory only and reset on restart. Charts retain real observations, so startup and upstream gaps remain visible.

Rolling spread statistics are computed server-side over the latest five minutes of executable two-sided quotes, with one-second shared caching per selection. Visible Execution pages fetch the summary every five seconds and only download the last minute of chart history. Reorganizations invalidate affected observations and cached summaries. Collection has a fixed RPC cost with zero viewers; additional viewers share it, while selected-topic delivery bandwidth still scales with active viewers. Depth remains demand-driven.

Historical aggregation runs on a read-only SQLite worker with a consistent WAL snapshot across both percentile passes. Results retain their existing per-window TTL, gain immutable revision URLs, and can be served from the optional analytics edge cache. Corrected marks receive new revisions. See [v2 realtime delivery](v2-realtime.md) for the protocol, benchmarks, deployment and rollback controls.
...
- **Workload-specific RPC pools** (`server/src/chain/rpc.ts`, `server/src/depth/`): **HOT** (`RPC_HTTP_URL` + `RPC_HTTP_BACKUP_URLS`) serves continuous block-triggered quotes and the fills tail, where distance to the tip matters most. Inside HOT, heads, per-adapter quotes and auxiliary traffic have separate transport lanes but share one breaker, so isolation cannot split a frame across different active providers. **DEPTH** (`RPC_DEPTH_URL` + `RPC_DEPTH_BACKUP_URLS`) lives in the depth child process and serves only demanded 25-point market curves; use a distinct endpoint so it cannot spend HOT's connection or rate-limit budget. Unset falls back to HOT for compatibility, with process isolation still intact. **ARCHIVE** (`RPC_ARCHIVE_URL` + `RPC_ARCHIVE_BACKUP_URLS`) serves volume backfill, markout onboarding, gas and `blockAtOrAfter`, where lag is irrelevant and retention is everything. It must meet the full requirement list above, historical `eth_getCode` included; the public endpoint does not, so ARCHIVE ships with no default backup. Leave `RPC_ARCHIVE_URL` unset and the archive client is the hot client.
...
- **RPC failover** (`server/src/chain/failover.ts`): each pool rides its own breaker in front of its ordered endpoint list. Three consecutive **endpoint-level** failures (network/timeout, 401/403/408 or 5xx) switch endpoints — JSON-RPC errors, 413 range caps and 429 throttles never do — and a 60s probe snaps back to the primary after two healthy checks. Hot reads are block-pinned and tails run at head−5, so quotes/fills/gap-fill can continue on a backup. Deep reads have a stricter contract: every result is accepted only if the archive pool stayed primary and healthy across the entire await. A transparent failover therefore discards even a successful backup result and holds the cursor, preventing mixed-provider or pruning-backup reads from becoming permanent history. The TopBar's amber/red `RPC` chip reflects the **hot** pool only; archive health is exposed as `state.rpcArchive` and both pools record every transition in `state.notes`, naming the pool (labels only — URLs embed keys).
