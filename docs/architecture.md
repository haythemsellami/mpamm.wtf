...
The cost is `DEPTH_SAMPLES / sizesUsd.length` ≈ 6× a four-pill market quote, so depth has a separate **process, scheduler and RPC pool**. The first `GET /api/depth/stream?market=` viewer starts that market; later viewers share it, and the last disconnect stops demand. New heads coalesce to the latest block while a pass is running, so stale work cannot queue. Only an already-serialized completed snapshot crosses to the main service, where SSE fanout is a cheap string write; `GET /api/depth?market=` is cache-only and never starts RPC work. The browser keeps the last good curve through worker/RPC failures, and depth state lives inside the curve component so an update does not re-render the rest of the dashboard. Set `RPC_DEPTH_URL` to a separately provisioned tip-fresh node to isolate provider quotas as well as the Node event loop; leaving it unset reuses the hot node and logs that reduced isolation. `DEPTH=off` disables the worker.
