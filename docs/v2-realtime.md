...
| Area | Behavior |
| --- | --- |
| Collection | One shared collector quotes every registered market and configured size, including baselines, independently of viewer count. Subscriptions select delivery, so opening another browser does not start another quote job. Depth retains its independent market demand and full size grid. |
...
Quotes remain pinned to an explicit block and a synchronously captured pair-terms CEX reference. Missing/expired results are absent, never replaced with stale prices. Fill finality margins, quote money math, schema version and atomic data/cursor transactions are unchanged. Legacy WebSocket clients retain full matrices; legacy `/api/quotes` and `/api/markets` return empty HTTP 200 snapshots during cold startup, then reuse or wait for a complete shared frame and return 503 if unavailable. These reads do not start additional RPC work.

Markets and baseline controls come from adapter discovery metadata, independently of the current quote selection. Alternate-size availability hints reuse the existing depth subscription. The shared collector retains full quote frames for 60 seconds and compact numeric spread observations for six minutes in memory. Expiry continues during upstream outages, even without viewers or API reads; nothing in these buffers is written to SQLite. Returning to Execution or switching selections replays the last minute, including observations collected without subscribers. Startup and real upstream gaps remain visible.

`GET /api/quotes/stats?market=&size=` returns percentiles, mean, population standard deviation and sample count over `(asOf - 5 minutes, asOf]`. Only executable two-sided quotes contribute. Results are cached per selection within a one-second bucket and refreshed by visible Execution pages every five seconds. Browsers download the summary, not five minutes of raw quotes. Reorganizations invalidate both buffers and cached summaries. Reconnect/history races preserve newer live frames and fills, including overlapping REST requests, worker fallback and rapid page restoration. Legacy sockets still wait for a fresh complete matrix.

Continuous collection has a fixed RPC cost even with zero Execution viewers; additional viewers share that work. Delivery bandwidth still grows with active viewers, using the existing selected topics and shared compression. Memory follows the retained time windows and observed block rate, rather than uptime or viewer count. A restart clears the buffers and starts a new warmup window.
...
Each adapter receives one quote call per evaluated block for all configured sizes and venue roles. Its outage reporter evaluates that complete result once. One failed market cannot flip a successfully quoting venue between unavailable and recovered, and canceled or rejected frames cannot commit partial health. An adapter retains its occupied slot until its work settles. The fill tail reads its boundary through the same isolated head lane as quote observation, retaining the five-block finality margin and cursor hold on failure.

## Shared execution history measurements

The [September 21 retention replay](benchmarks/execution-history-runtime.json) uses the recorded 213-row matrix for one synthetic hour at 300ms cadence on Node 24.11.1. At 6, 12 and 60 minutes it retains exactly 200 full chart frames and 1,200 compact spread frames. The numeric spread payload stays at 3,052,800 bytes; this excludes JavaScript object overhead and is not process RSS. Recording a frame measured 0.033ms p95; an uncached selection summary measured 1.44ms p95, and repeated reads within the shared cache bucket measured 0.015ms p95. Timings exclude quote generation and RPC. Reproduce with `node --import tsx scripts/benchmark-execution-history.ts` from `server/`.

The [live retention check](benchmarks/execution-history-live.json) ran for 310 seconds with zero WebSocket subscribers, Metric and CEX references, and backfills/depth/gas disabled. Independently recorded observations reproduced both BTC/USDC $1k and ETH/USDC $10k five-minute summaries to within 2.7e-15 bps. Each final window contained 276 observed frames, including 171 executable Metric samples; chart history retained only the latest 38 frames spanning 58.9 seconds. This Node 26.9.0 local integration run used HTTP heads and ended with 19.2% coverage over the last minute: it validates continuous collection and aggregate correctness, not complete block coverage or production capacity. Unit and browser checks run on the production Node 24 major version.

## Earlier delivery measurements
...
These measurements predate continuous collection. The selected/idle simulator row records the former demand policy; the current collector always builds the shared matrix. Delivery remains filtered per topic. `benchmark:runtime` now measures continuous collection explicitly.
...
Browser quote-history downloads, statistics polling and chart buffers are active only on Execution; the server continues collecting independently. Browser checks open Markouts, Volume and Leaderboard directly, visit Execution and return: each non-Execution entry makes zero quote-history requests, while Execution fetches history successfully and renders its charts. They also verify replay of blocks collected on another page and shared statistics in a new browser session. Unit checks cover selection/registry changes, reconnects, reorganization invalidation and history responses completing after leaving and reopening Execution.
...
Subscription changes immediately discard queued frames and release the backlog budget for removed topics, including the initial snapshot being compressed. Retained topics keep their ordered events. A compression job cannot deliver into a replacement subscription for the same topic. Regressions cover retained fills, ten rapid market switches and resubscription while another peer keeps the shared topic alive. The [subscription delivery replay](benchmarks/v2-review-subscriptions-delivery.json) retained every frame and the 98.6% quote-byte reduction: 184.0 KB/s per viewer raw versus 2.60 KB/s selected gzip at 100 viewers, with 8ms receipt p95. The initial snapshot used one encode and 806 payload bytes per viewer versus 1,695 JSON bytes.

The SharedWorker retains individual component identities within each tab. Subscription updates replay cached data only to new consumers or newly requested topics. Live frames still share one structured clone per tab, with consumer ids attached. The [worker replay check](benchmarks/v2-review-worker-replay.json) compares the actual page, worker and hub modules at `55be501` against the updated modules using loopback ports and deterministic frames. Ten depth-hint mount/unmount cycles previously delivered 60 duplicate callbacks to existing components; the update delivers zero. All ten joining hints still receive their cached snapshot. Worker-to-page envelope messages fall from 60 to 10 during those cycles; a subsequent 30-frame live stream still sends 30 messages and delivers every expected callback with one socket. This measures duplicate delivery, not CPU, latency or Render egress.
...
Full legacy snapshots require every requested adapter to complete; deadline, rejected-task and occupied-slot frames remain available on v2 with their existing telemetry but cannot complete a legacy handoff or replace its complete matrix. `/api/bootstrap`, `/api/fills` and both leaderboard endpoints return retryable HTTP 503 until persisted history has loaded. The leaderboard cannot compute or cache a premature empty aggregate. The browser retries independently of socket reconnection and retains fills arriving during failed requests. Legacy quote startup endpoints keep their empty HTTP 200 contract. State cannot regress behind bootstrap or a newer stream frame, and SharedWorker replay excludes aged snapshots and disconnected caches.
...
Hanji validates its complete fixed market list before committing discovery. A failed read or token mismatch throws before either the catalog or decode map is replaced, so a partial discovery cannot remove an admitted market. Five regressions replay [live configuration recorded at block 105857551](../server/src/venues/__tests__/fixtures/hanji-config.json), covering partial/empty responses, token mismatch, transport failure and initial recovery. All six markets and fill sources survive a failed refresh, and scoped quote requests still reach the retained market.

Lunarbase retains previously admitted markets in its discovered catalog while quarantine removes the active quote/fill caches. A [live catalog comparison](benchmarks/v2-review-quarantine-catalog-live.json) at block 105686305 used actual discovery reads and injected an empty implementation slot locally for one pass. The previous adapter lost MON/USDC from its catalog; the updated adapter retained it. Both withheld quarantined quotes/fill sources and recovered after restoring the real slot. The pool was paused; this verifies catalog and gating behavior, not executable pricing or a real on-chain upgrade.
